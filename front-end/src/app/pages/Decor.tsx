...
export default function Decor() {
  ...
  const { items: decorItems, isLoading, errorMessage } = useCategoryItems('decor');
  const bookedIds = useBookedEntities(filters.date);
  const filteredDecor = filterItems(decorItems, filters, bookedIds);

  useEffect(() => {
    if (filters.selectedCategory !== 'decor') {
      updateFilters({ selectedCategory: 'decor' });
    }
  ...
  return (
    <div className="min-h-screen bg-[#F7F7F9]">
      <div className="container mx-auto max-w-6xl px-4 py-8">
        <div className="relative mb-8 overflow-hidden rounded-2xl border border-[#EABAB0]/30 bg-white">
          ...
          <div className="relative flex items-center justify-between px-6 py-5">
            <div>
              ...
              <h1 className="text-2xl font-bold text-slate-800">Event Decor</h1>
              <p className="mt-0.5 text-sm text-slate-400">Transform your venue with beautiful styling</p>
            </div>
            <div className="hidden sm:flex size-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#875A6B]/10 to-[#C9995A]/10 border border-[#EABAB0]/30">
              <Sparkles className="size-6 text-[#875A6B]" />
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
          <p className="mb-3 text-xs font-bold uppercase tracking-widest text-slate-400">Refine decor</p>
        ...
        {isLoading && (
          <div className="mb-6 flex items-center justify-center gap-3 py-10">
            ...
            <span className="text-sm text-[#875A6B]/70 font-medium">Loading decor options…</span>
          </div>
        )}
        ...
        {filteredDecor.length === 0 ? (
          <div className="rounded-2xl border border-slate-200 bg-white py-16 text-center">
            <p className="text-base font-semibold text-slate-600 mb-1">No decor options match your filters</p>
            <p className="text-sm text-slate-400">Try adjusting your search criteria</p>
          </div>
        ) : (
          <>
            <p className="mb-4 text-sm font-medium text-slate-500">
              <span className="font-bold text-slate-700">{filteredDecor.length}</span> {filteredDecor.length === 1 ? 'result' : 'results'} found
            </p>
